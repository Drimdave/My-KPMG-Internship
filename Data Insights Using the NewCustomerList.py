...
# I would be doing the following Data Exploration to target the high value customers based on the New Customer List:
# 1. Age distributions
...
df22 = pd.read_excel(KPMG, 'NewCustomerList')
...
df2.rename(columns={"Note: The data and information in this document is reflective of a hypothetical situation and client. This document is to be used for KPMG Virtual Internship purposes only. ":"fname"}, inplace = True)
df2.rename(columns={"Unnamed: 1":"lname",
                   "Unnamed: 2":"gender",
                   "Unnamed: 3":"3y_bike_purchases",
                   "Unnamed: 4":"DOB",
                   "Unnamed: 5":"JT",
                   "Unnamed: 6":"Category"}, inplace = True)
df2.rename(columns={"Unnamed: 7":"wealth_segement",
                   "Unnamed: 8":"D_Indicator",
                   "Unnamed: 9":"owns_car",
                   "Unnamed: 10":"tencure",
                   "Unnamed: 11":"address",
                   "Unnamed: 12":"postcode"}, inplace = True)
df2.rename(columns={"Unnamed: 13":"state",
                   "Unnamed: 14":"country",
                   "Unnamed: 15":"prop_val",
                   "Unnamed: 21":"rank",
...
def check_unique():
    ...
    for i in range(2, 23):
        ct = 0
        if NaNlist[i] > 0:
            ct = len(df2[temp[i]].unique()) - 1
        else:
            ct = len(df2[temp[i]].unique())
        mydict[temp[i]] = (ct, df2[temp[i]].unique())
    return mydict
...
bike = [0,0,0]
df2[['3y_bike_purchases']] = df2[['3y_bike_purchases']].apply(pd.to_numeric)
bike[0] += df2['3y_bike_purchases'][df2['gender'] == 'Female'].sum() 
bike[1] += df2['3y_bike_purchases'][df2['gender'] == 'Male'].sum()
bike[2] += df2['3y_bike_purchases'][df2['gender'] == 'U'].sum()
...
for i in range(1, lenn):
    #type is date
    if isinstance(df2["DOB"][i], datetime.date):
        tl = len(df2["DOB"][i].ctime().split(" "))
        print(int(df2["DOB"][i].ctime().split(" ")[tl-1]))
        df2["Age"][i] += int(2019 - int(df2["DOB"][i].ctime().split(" ")[tl-1]))
    #type is string
    elif isinstance(df2["DOB"][i], str):
        tl = len(df2["DOB"][i].split("-"))
        df2["Age"][i] += int(2019 - int(df2["DOB"][i].split("-")[0])) 
...
stdv    = []
for each in df2['gender']:
    if each[0] == "F" and df2['Age'][j] != 0:
        ss   += df2['Age'][j]
        stdv.append((df2['Age'][j]))
        ngenage[0] += 1
    elif each[0] == "M" and df2['Age'][j] != 0:
        ss += df2['Age'][j]
        stdv.append((df2['Age'][j]))
        ngenage[1] += 1
    elif df2['Age'][j] != 0: 
...
jjj = 1
for each in df2['gender']:
    temp = int(df2['Age'][jjj])
    bkt  = int(df2['3y_bike_purchases'][jjj])
...
for each in df2['wealth_segement']:
    temp = int(df2['Age'][jjj])
